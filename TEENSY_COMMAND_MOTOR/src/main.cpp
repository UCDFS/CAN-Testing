...
void executeStep(int step) {
  currentStep = step;
  switch (step) {
    case 1: requestStatusCyclic(100); requestSpeedCyclic(100); break;
...
void setup() {
  ...
  // --- Step 1: start cyclic requests, wait for BAMOCAR ---
  waitForButton("STEP 1: START");
  ...
  // --- Step 2: request DC bus voltage ---
  waitForButton("STEP 2: DC BUS");
  executeStep(2);
  delay(200);

  // --- Step 3: clear errors ---
  waitForButton("STEP 3: CLR ERR");
  executeStep(3);
  delay(200);

  // --- Step 4: configure CAN timeout ---
  waitForButton("STEP 4: CAN TMO");
  executeStep(4);
  delay(200);

  // --- Step 5+6: enable drive, zero torque ---
  waitForButtonHeld("STEP 5: ENABLE");
  ...
  // --- Wait for pedal release (automatic, no button needed) ---
...
void loop() {
  ...
  // --- Torque control ---
  if (currentStep == 7 && driveEnabled && millis() - lastTorqueSend >= 20) {
